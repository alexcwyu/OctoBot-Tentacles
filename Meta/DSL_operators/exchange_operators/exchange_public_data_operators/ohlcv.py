...
def create_ohlcv_operators(
    ...
    def _get_candle_manager() -> octobot_trading.exchange_data.CandlesManager:
        return octobot_trading.api.get_symbol_candles_manager(
            octobot_trading.api.get_symbol_data(exchange_manager, symbol, allow_creation=False), time_frame
        )
    ...
    class _ClosePriceOperator(OHLCVOperator):
        ...
        async def initialize(self) -> None:
            ...
            self.value = _get_candle_manager().get_symbol_close_candles(-1)
    ...
    class _OpenPriceOperator(OHLCVOperator):
        ...
        async def initialize(self) -> None:
            await super().initialize()
            self.value = _get_candle_manager().get_symbol_open_candles(-1)
